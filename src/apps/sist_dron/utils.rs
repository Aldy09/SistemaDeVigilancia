...
fn load_id_ip_and_port() -> Result<(u8, String, u16), Error> {
    ...
    let id = argv[1].parse::<u8>().map_err(|_| Error::new(ErrorKind::InvalidInput, "El id proporcionado no es válido"))?;
    let ip = &argv[2];
    let port = argv[3].parse::<u16>().map_err(|_| Error::new(ErrorKind::InvalidInput, "El puerto proporcionado no es válido"))?;
...
pub fn get_id_and_broker_address() -> Result<(u8, SocketAddr), Error> {
    ...
    let broker_addr = addr.parse().map_err(|_| Error::new(ErrorKind::InvalidInput, "Dirección no válida"))?;
